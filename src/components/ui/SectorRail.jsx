...
export default function SectorRail() {
  ...
  const idx = Math.max(
    0,
    SECTORS.findIndex((s) => s.id === active),
  );
  ...
  return (
    ...
    >
      <div className="relative flex flex-col items-end">
        ...
        <div
          className="absolute right-[5px] top-3 w-[2px] bg-gradient-to-b from-plasma to-plasma-mid shadow-[0_0_8px_rgb(var(--accent)/0.6)] transition-all duration-700"
          style={{
            height: `calc(${(idx / (SECTORS.length - 1)) * 100}% - ${(idx / (SECTORS.length - 1)) * 24}px)`,
          }}
        />
        {SECTORS.map((s, i) => {
            ...
            >
              <span
                className={`whitespace-nowrap font-code text-[9px] tracking-[0.2em] transition ${
                  isActive
                    ? "rounded border border-plasma/50 bg-space-deep/85 px-2 py-1 text-plasma shadow-[0_0_14px_rgb(var(--accent)/0.3)]"
              ...
              <span
                className={`relative rounded-full transition-all duration-300 ${
                  isActive
                    ? "rail-pulse h-4 w-4 bg-plasma shadow-[0_0_16px_rgb(var(--accent)/0.9)]"
                    : passed
                      ? "h-3 w-3 bg-plasma-mid shadow-[0_0_8px_rgb(var(--accent2)/0.6)]"
