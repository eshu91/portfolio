...
function StarLayer({ count, radius, thickness, baseScale, drift }) {
  ...
  useFrame((state) => {
    ...
    if (group.current) group.current.rotation.y = state.clock.elapsedTime * drift
  })
...
export default function Starfield() {
  ...
  return (
    <>
      ...
      <StarLayer count={Math.floor(6000 * mult)} radius={160} thickness={60} baseScale={1.6} drift={0.004} />
      <StarLayer count={Math.floor(4000 * mult)} radius={110} thickness={40} baseScale={2.2} drift={0.008} />
      <StarLayer count={Math.floor(2000 * mult)} radius={70} thickness={30} baseScale={3.0} drift={0.013} />
    </>
  )
}
