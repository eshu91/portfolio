...
function Station({ project, position, index }) {
  ...
  useFrame((state, delta) => {
    ...
    if (wire.current) wire.current.material.opacity = 0.25 + Math.abs(Math.sin(t * 1.4)) * 0.3
  })
  ...
  return (
    <group position={position}>
      <group ref={group}>
        {locked ? (
          ...
          <mesh ref={wire}>
            ...
            <meshBasicMaterial color="#F8D866" wireframe transparent opacity={0.35} />
          </mesh>
        ) : (
          <>
            {/* habitat core */}
            <mesh>
              ...
              <meshStandardMaterial color="#083344" roughness={0.5} metalness={0.4}
                emissive={new THREE.Color('#0e5268')} emissiveIntensity={0.5} />
            </mesh>
            {/* docking ring */}
            <mesh ref={ringA} rotation={[Math.PI / 2, 0, 0]}>
              <torusGeometry args={[1.9, 0.06, 12, 80]} />
              <meshStandardMaterial color="#22D3EE" emissive={new THREE.Color('#22D3EE')}
      ...
      <Html center position={[0, locked ? 2.2 : 2.9, 0]} distanceFactor={22} style={{ pointerEvents: 'none' }}>
        <p className={`whitespace-nowrap font-code text-[11px] tracking-widest ${locked ? 'text-stargold/70' : 'text-plasma'}`}>
    ...
    <group>
      {stationed.map((p, i) => (
        <Station
          ...
          project={p}
          index={i}
          position={[CORRIDOR_X + i * GAP, (i % 2) * 1.2 - 0.6, (i % 3 - 1) * 2]}
        />
      ))}
    </group>
  )
