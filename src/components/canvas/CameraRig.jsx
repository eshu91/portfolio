...
export default function CameraRig() {
  ...
  useFrame((state, delta) => {
    ...
    // supernova camera shake (decaying jitter)
    const seq = useStore.getState().novaSeq
    if (seq !== shake.current.seq) {
      shake.current.seq = seq
      shake.current.t = 0.55
    }
    let jx = 0, jy = 0
    if (shake.current.t > 0) {
      shake.current.t = Math.max(0, shake.current.t - delta)
      const amp = shake.current.t * 0.9
      jx = (Math.random() - 0.5) * amp
      jy = (Math.random() - 0.5) * amp
    }

    const t = state.clock.elapsedTime
    const px = pos.current.x + state.pointer.x * 1.1 + Math.sin(t * 0.1) * 0.35 + jx
    const py = pos.current.y + state.pointer.y * 0.7 + Math.cos(t * 0.13) * 0.25 + jy
