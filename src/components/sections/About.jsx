...
function Highlighted({ code }) {
  return code.split(TOKEN_RE).map((part, i) => {
    if (part.startsWith('"')) return <span key={i} className="text-stargold">{part}</span>
    if (KEYWORDS.has(part)) return <span key={i} className="text-plasma-mid">{part}</span>
...
export default function About() {
  return (
    <section id="about" className="dom-layer min-h-screen flex items-center justify-center px-4 py-24">
      <motion.div
      ...
      >
        ...
        <div className="flex items-center gap-2 border-b border-space-grad/70 bg-space-mid/50 px-4 py-2.5 font-code text-xs">
          ...
          <span className="ml-2 text-plasma/80 tracking-wider">observatory://snova.profile</span>
