...
function ProjectCard({ p, i }) {
  ...
  const ACCENTS = [theme.accent, theme.accent2, theme.gold];
  if (p.locked) {
    return (
      <motion.div
      ...
      >
        <p className="font-code text-sm tracking-widest text-stargold/90">
          {p.title}
        </p>
  ...
  return (
    <motion.div
      ...
      viewport={{ once: true, amount: 0.2 }}
      transition={{ delay: (i % 3) * 0.08 }}
      className="group flex min-h-[170px] flex-col rounded-lg border border-space-grad bg-space-deep/70 p-5 backdrop-blur-sm transition hover:border-plasma-mid/70 hover:shadow-[0_0_28px_rgb(var(--accent2)/0.25)]"
    >
      <div className="flex items-start justify-between gap-3">
        <p className="font-code text-sm text-plasma">{p.title}</p>
        {p.featured && (
          <span className="font-code text-[10px] text-stargold">★</span>
        )}
      </div>
        ...
        <div className="mt-3 flex gap-4 font-code text-[11px]">
          {p.github && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-plasma-mid hover:text-plasma"
            >
              ⌥ github
            </a>
          )}
          {p.demo && (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="text-stargold/80 hover:text-stargold"
            >
              ⏵ live demo
            </a>
          )}
...
export default function Projects() {
  ...
  const list = useMemo(() => {
    if (filter === "featured")
      return projects.filter((p) => p.featured && !p.locked);
          ...
          {FILTERS.map((f) => (
            <button
              ...
              className={`rounded-full border px-4 py-1.5 font-code text-[11px] tracking-wider transition ${
                filter === f.id
                  ? "border-plasma bg-space-mid text-plasma shadow-[0_0_16px_rgb(var(--accent)/0.35)]"
