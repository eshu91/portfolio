      ...
      {roadmap.map((m) => {
        ...
        return (
          <li key={`${m.year}-${m.title}`} className="relative">
            ...
            />
            <p className="font-code text-[11px] tracking-widest" style={{ color: s.color }}>
              ⟪ {m.year} ⟫ {s.icon}
            </p>
...
export default function Roadmap() {
  ...
  return (
    <section id="roadmap" className={`dom-layer px-6 ${fallback ? 'min-h-screen py-24' : 'h-[150vh]'}`}>
      <div className={fallback ? '' : 'sticky top-0 flex h-screen flex-col items-center pt-16 pointer-events-none'}>
        ...
        <p className="mt-2 text-center font-code text-[11px] text-white/50">
          {fallback ? 'flight path so far — and where the ship is headed' : 'the constellation below charts the flight path'}
        </p>
      </div>
