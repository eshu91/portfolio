  ...
  return (
    <div className="mx-auto mt-10 grid max-w-3xl gap-4 sm:grid-cols-2">
      {skills.map((s) => (
        <div
          key={s.id}
          className="rounded-lg border border-space-grad bg-space-mid/40 p-4"
        >
      ...
      <div className="rounded-lg border border-space-grad bg-space-mid/40 p-4 sm:col-span-2">
        <p className="font-code text-sm text-plasma">🪨 Tools asteroid belt</p>
        <p className="mt-2 font-code text-[11px] leading-relaxed text-white/70">
          {asteroidBelt.tools.join(" · ")}
        </p>
      </div>
    </div>
  );
...
export default function Skills() {
  ...
  return (
    <section
      id="skills"
      className={`dom-layer px-6 ${fallback ? "min-h-screen py-24" : "h-[200vh]"}`}
    >
      <div
        className={
          fallback
